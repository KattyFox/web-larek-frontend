...
- **API** — обёртка над `fetch`, реализованная в классе `Api`.
...
### 📄 Описание
`Api` — это универсальная обёртка над стандартным `fetch`, обеспечивающая удобную работу с REST API: запросы GET, POST, PATCH, DELETE и др.
...
```
- `baseUrl: string` — базовый URL, к которому будут добавляться эндпоинты (например: `https://api.example.com/v1`).
- `options: RequestInit` *(опционально)* — глобальные опции для всех запросов (заголовки, авторизация и т.д.).


### 📌 Методы


- `get(endpoint: string, options?: RequestInit)` — выполняет `GET` запрос.
- `post(endpoint: string, data: any, options?: RequestInit)` — выполняет `POST` запрос с телом.
- `put(endpoint: string, data: any, options?: RequestInit)` — выполняет `PUT` запрос.
- `patch(endpoint: string, data: any, options?: RequestInit)` — выполняет `PATCH` запрос.
- `delete(endpoint: string, options?: RequestInit)` — выполняет `DELETE` запрос.


### 📤 Возвращаемое значение
Каждый метод возвращает `Promise<any>`, который резолвится результатом `response.json()` либо отклоняется при ошибке сети/сервера.


### 🧩 Пример использования
```ts
const api = new Api("https://api.example.com/v1", {
headers: {
...
**Методы:**  
- `create()` — вызывает `super.create()`, добавляет обработку кнопок cash/card и ввода адреса  
- `validate()` — использует `commonValidate()` и дополнительную проверку выбора способа оплаты  
- `handleSubmit()` — диспатчит событие `payViewDone`  

**События (кастомные):**  
- Клик по кнопке submit (валидно) → `dispatch("payViewDone")`  
...
**Методы:**  
- `create()` — вызывает `super.create()`, добавляет обработку полей email/phone  
- `validate()` — использует `commonValidate()` и HTML5-проверку email и телефона  
- `handleSubmit()` — диспатчит событие `contactViewDone`  

**События (кастомные):**  
- Клик по кнопке submit (валидно) → `dispatch("contactViewDone")`  
...
<img width="967" height="380" alt="image" src="https://github.com/user-attachments/assets/fd041b65-7c1c-4518-81ee-b9988f92130e" />
...
<img width="1005" height="270" alt="image" src="https://github.com/user-attachments/assets/a5d9a18b-8548-454a-ae3b-6765e579eb1c" />
...
<img width="940" height="310" alt="image" src="https://github.com/user-attachments/assets/3e419eba-e7ab-4ac4-90f5-1e1f7b370866" />
...
<img width="1080" height="375" alt="image" src="https://github.com/user-attachments/assets/ceabea4b-3874-46af-bc6a-02ae0a07563f" />


<img width="1187" height="765" alt="image" src="https://github.com/user-attachments/assets/ba738079-e992-4486-bbbe-47fad976b1a5" />
...
- После рефакторинга PayView и ContactView наследуются от Form, но их события (payViewDone, contactViewDone) и взаимодействие с OrderController не изменились.
---
...
<img width="1280" height="641" alt="image" src="https://github.com/user-attachments/assets/e86a47aa-33ea-4323-8d59-4fbf1192fd8d" />
...
В этой схеме видно, что **OrderController.postToServer()** использует класс **Api** для отправки заказа на сервер.
